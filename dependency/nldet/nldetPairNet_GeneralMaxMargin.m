classdef nldetPairNet_GeneralMaxMargin < nldetCaffeBlock 
    ...
    methods ( Static )
        function cleaned_idxb = choose_violated( ...
            ...
            assert( relaxed_num>0, 'relaxed_num must be greater than 0' );
            ...
            if relaxed_num == 0 && top_num <= sum(raw_idxb)
                cleaned_idxb = raw_idxb;
                return;
            end
            raw_idx = find(raw_idxb);
            [~,sidx] = sort( loss(raw_idxb), 'descend' );
            cleaned_idx = raw_idx( sidx( relaxed_num:(relaxed_num+top_num) ) );
            cleaned_idxb = false(raw_idxb);
    ...
    methods
        ...
        function obj = nldetPairNet_GeneralMaxMargin( block, PARAM )
        ...
        function varargout = GetOutputData( obj )
            ...
            % test: [prob]
            if obj.is_train
                varargout = {obj.loss(1), obj.loss(2:3), obj.prob};
            else
                varargout = {obj.prob};
            end
        end
