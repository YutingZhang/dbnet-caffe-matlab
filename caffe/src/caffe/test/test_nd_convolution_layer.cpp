...
class ndConvolutionLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  ndConvolutionLayerTest()
  ...
  virtual ~ndConvolutionLayerTest() {
    delete blob_bottom_;
...
TYPED_TEST_CASE(ndConvolutionLayerTest, TestDtypesAndDevices);

TYPED_TEST(ndConvolutionLayerTest, TestSetup) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  shared_ptr<Layer<Dtype> > layer(
      new ndConvolutionLayer<Dtype>(layer_param));
  ...
  layer.reset(new ndConvolutionLayer<Dtype>(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), 2);
...
TYPED_TEST(ndConvolutionLayerTest, TestSimplendConvolution) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  LayerParameter layer_param;
  ...
  //
  convolution_param->set_parallel_batch_size(2);
  //
  shared_ptr<Layer<Dtype> > layer(
      new ndConvolutionLayer<Dtype>(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
...
TYPED_TEST(ndConvolutionLayerTest, Test1x1ndConvolution) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ndConvolutionLayerTest, TestSimplendConvolutionGroup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ndConvolutionLayerTest, TestSobelndConvolution) {
...
TYPED_TEST(ndConvolutionLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
  ...
  convolution_param->set_parallel_batch_size(2);
  //
  ndConvolutionLayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
      this->blob_top_vec_);
}

TYPED_TEST(ndConvolutionLayerTest, Test1x1Gradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(ndConvolutionLayerTest, TestGradientGroup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
