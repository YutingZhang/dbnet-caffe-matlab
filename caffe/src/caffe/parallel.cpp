...
// apply op from blobs to buffer, blobs are ptrs
...
GPUParams<Dtype>::GPUParams(shared_ptr<Solver<Dtype> > root_solver, int device)
  ...
  // copy data and diff in root_solver into device buffer
...
void GPUParams<Dtype>::configure(Solver<Dtype>* solver) const {
  ...
  // apply the data and gradient into gpu
...
// construct p2p synchronization among gpus?
template<typename Dtype>
P2PSync<Dtype>::P2PSync(shared_ptr<Solver<Dtype> > root_solver,
    ...
    : GPUParams<Dtype>(root_solver, param.device_id()),
      ...
      solver_() {
#ifndef CPU_ONLY
  // the initial device works like a host who spread the job
  int initial_device;
  CUDA_CHECK(cudaGetDevice(&initial_device));
  // the code below will run on the device "self"
  const int self = param.device_id();
  CUDA_CHECK(cudaSetDevice(self));
  device_id = self;

  // define the solver type of the device
  if (parent == NULL) {
    solver_ = root_solver;
  } else {
...
//  CHECK(false);
  ...
  if (parent_) {
    // pop will be blocked if the queue is empty
  ...
  for (int i = children_.size() - 1; i >= 0; i--) {
    ...
    // start training after the data all finish copying
...
template<typename Dtype>
void P2PSync<Dtype>::on_batch(Dtype* batch_data_, const int size){
#ifndef CPU_ONLY
    // allocate memory for copying from child
    Dtype *child_data_;
    if (!children_.empty()) CUDA_CHECK(cudaMalloc(&child_data_, size * sizeof(Dtype)));

    // collect and combine data from child
    for (int i = 0; i < children_.size(); ++i){
        Dtype *src = batch_queue_.pop();
        Dtype *dst = child_data_;

        //caffe_gpu_memcpy(size * sizeof(Dtype), src, dst);
        CUDA_CHECK(cudaMemcpyAsync(dst, src, size * sizeof(Dtype),
                    cudaMemcpyDeviceToDevice, cudaStreamDefault));
        CUDA_CHECK(cudaStreamSynchronize(cudaStreamDefault));
        ...
        src = child_data_;
        dst = batch_data_;
        caffe_gpu_add(size, src, dst, dst);
    }

    if (parent_){
        parent_->batch_queue_.push(batch_data_);

        // then listen from parent data transmitted back
        Dtype *src = batch_queue_.pop();
        Dtype *dst = batch_data_;
        ...
        parent_->finish_queue_.push(true);
    }
    else{
        caffe_gpu_scal(size, Dtype(1.0 / Caffe::solver_count()), batch_data_);
    }

    for (int i = 0; i < children_.size(); ++i){
        children_[i]->batch_queue_.push(batch_data_);
    }
    
    // wait for child functions to finish copying data
    for (int i = 0; i < children_.size(); ++i){
        finish_queue_.pop();
    }
    // free the memory
    if (!children_.empty()) CUDA_CHECK(cudaFree(child_data_));
#endif
}

// opposite way to on_start()
template<typename Dtype>
...
// use compute to pair gpu; assign parents according to pairs (tree topology)
template<typename Dtype>
