...
void EltwiseLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* coeffs_data = coeffs_.cpu_data(); // CPU data
  switch (op_) {
  ...
  case EltwiseParameter_EltwiseOp_SUM:
    ...
    for (int i = 0; i < bottom.size(); ++i) {
      caffe_gpu_axpy(count, coeffs_data[i], bottom[i]->gpu_data(), top_data);
    }
    break;
...
void EltwiseLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  const Dtype* coeffs_data = coeffs_.cpu_data(); // CPU DATA
  Dtype* coeffs_diff = coeffs_.mutable_gpu_diff();
    ...
    if (propagate_down[i]) {
      ...
      case EltwiseParameter_EltwiseOp_SUM:
        if (coeffs_data[i] == Dtype(1.)) {
          ...
          caffe_gpu_scale(count, coeffs_data[i], top_diff, bottom_diff);
        }
        break;
      case EltwiseParameter_EltwiseOp_MAX:
