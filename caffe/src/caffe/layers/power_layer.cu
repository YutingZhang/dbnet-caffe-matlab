...
void PowerLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if ( learnable_param_ ) {
	  ...
	  if ( this->param_propagate_down_[PARAM_SCALE] && learnable_scale_ ) {
		  const Dtype* bottom_data = bottom[0]->gpu_data();
		  Dtype* scale_diff = this->blobs_[PARAM_SCALE]->mutable_gpu_diff();
		  caffe_gpu_gemv( CblasNoTrans, 1, count,
				  Dtype(1.), bottom_data, top_diff, Dtype(1.),
				  scale_diff );
	  }
	  if ( this->param_propagate_down_[PARAM_SHIFT] && learnable_shift_ ) {
		  SyncedMemoryGuard sum_multiplier_holder = sum_multiplier_.hold_data();
		  Dtype* shift_diff = this->blobs_[PARAM_SHIFT]->mutable_gpu_diff();
		  Dtype* sum_mult   = sum_multiplier_.mutable_gpu_data();
		  caffe_gpu_set(count, Dtype(1), sum_mult );
		  caffe_gpu_gemv( CblasNoTrans, 1, count,
