...
void WindowSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->layer_param_.image_data_param().shuffle()) {
	...
	int shuffle_seed = this->layer_param_.image_data_param().shuffle_seed();
    const unsigned int prefetch_rng_seed = (shuffle_seed>=0)?
    		((unsigned int)shuffle_seed):caffe_rng_rand();
    LOG(INFO) << "WINDOW SEG DATA LAYER: SHUFFLE SEED: " << shuffle_seed;
