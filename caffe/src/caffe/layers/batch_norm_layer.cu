...
void BatchNormLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (use_global_stats_) {
    ...
    if (Layer<Dtype>::cur_device && across_gpus_){
        Layer<Dtype>::cur_device->on_batch(mean_.mutable_gpu_data(), mean_.count());
    }
  }
  ...
  if (!use_global_stats_) {
    ...
    if (Layer<Dtype>::cur_device && across_gpus_){
        Layer<Dtype>::cur_device->on_batch(variance_.mutable_gpu_data(), variance_.count());
    }
