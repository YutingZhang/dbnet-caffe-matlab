...
void Vol2colLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  parallel_batch_size_ = (parallel_batch_size_>=1)?(parallel_batch_size_):(1);
  CHECK( !(num_%parallel_batch_size_) ) << "num is not divisible by parallel_batch_size";
}

template <typename Dtype>
void Vol2colLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void Vol2colLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num_; n+=parallel_batch_size_) {
    ...
    vol2col_cpu(bottom_data + n * bottom_dim_, num_spatial_axes_,
...
template <typename Dtype>
void Vol2colLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  const Dtype* top_diff = top[0]->cpu_diff();
  Dtype* bottom_diff = bottom[0]->mutable_cpu_diff();
  for (int n = 0; n < num_; n+=parallel_batch_size_) {
    col2vol_cpu(top_diff + n * top_dim_, num_spatial_axes_,
        bottom[0]->shape().data() + channel_axis_,
        top[0]->shape().data() + channel_axis_,
        kernel_shape_.cpu_data(), pad_.cpu_data(), stride_.cpu_data(),
...
#ifdef CPU_ONLY
STUB_GPU(Vol2colLayer);
#endif

INSTANTIATE_CLASS(Vol2colLayer);
REGISTER_LAYER_CLASS(Vol2col);
