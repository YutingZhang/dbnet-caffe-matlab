...
void SoftmaxWithLossLayer<Dtype>::Reshape(
  ...
  CHECK_EQ(bottom[1]->num_axes()<=1 || bottom[1]->shape(softmax_axis_), 1) <<
		  "label should have only 1 channel";
  if (bottom.size() == 3) // bottom[2] is for weight
  {
	  CHECK(bottom[1]->shape() == bottom[2]->shape()) << 
          "Weight ( bottom[2] ) andn Label ( bottom[1] ) should have the same shape" ;
  }
}
...
void SoftmaxWithLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      if ( (has_ignore_label_ && label_value == ignore_label_) || channels<=label_value) {
        continue;
      }
...
void SoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    Dtype loss_weight = top[0]->cpu_diff()[0] /
                        get_normalizer(normalization_, weights_sum);
