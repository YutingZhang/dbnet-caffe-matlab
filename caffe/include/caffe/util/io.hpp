...
cv::Mat ReadImageToCVMat(const string& filename,
    const int height, const int width, const bool is_color,
    int* img_height, int* img_width=NULL);
...
template <typename Dtype, class Vec_T>
void ArrayToGZ( const std::string& fn, Vec_T dims, const Dtype* data ) {
	std::vector<size_t> dims1( dims.begin(), dims.end() );
	ArrayToGZ_Internal( fn, dims1, data );
}

template <typename Dtype, class Vec_T>
void ArrayToGZ( Vec_T dims, const Dtype* data, const std::string& fn ) {
	ArrayToGZ( fn, dims, data );
}
