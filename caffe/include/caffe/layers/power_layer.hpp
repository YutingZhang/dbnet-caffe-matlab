...
class PowerLayer : public NeuronLayer<Dtype> {
 public:
  ...
  explicit PowerLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param),
        power_(param_val_[PARAM_POWER]),
        scale_(param_val_[PARAM_SCALE]),
        shift_(param_val_[PARAM_SHIFT]) {}
 ...
 protected:
  ...
  bool learnable_param_;
  bool learnable_power_;
  bool learnable_scale_;
  bool learnable_shift_;
  bool nonnegative_scale_;
  bool nonnegative_shift_;
  void sync_param();

  DynamicBlob<Dtype> sum_multiplier_;
};
