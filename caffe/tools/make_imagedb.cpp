...
int main(int argc, char** argv) {
  ...
  LOG(INFO) << "Make imagedb dir";
  string root_dir( argv[1] );
  string db_dir( argv[3] );
  {
      boost::filesystem::path db_dir_path( db_dir );
      boost::filesystem::create_directories( db_dir_path );
  }

  LOG(INFO) << "Creat index file";
  ...
  for ( i = 0; i<lines.size(); ++i ) {
      ...
      std::string cur_fn = ( root_dir + "/" + lines[i].first );
	  std::ifstream cur_infile( cur_fn.c_str(), std::ios_base::binary );

      if (!cur_infile)
          LOG(FATAL) << "Cannot open : " << cur_fn;
