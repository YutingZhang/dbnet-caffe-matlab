classdef Blob < handle
  ...
  methods
    ...
    function data = get_device_data(self)
      device_mode = caffe_('get_mode');
      if strcmp(device_mode, 'gpu');
        data = caffe_('blob_get_gpu_data', self.hBlob_self);
      else
    ...
    function data = get_gpu_data(self)
      data = caffe_('blob_get_gpu_data', self.hBlob_self);
    end
    ...
    function diff = get_device_diff(self)
      device_mode = caffe_('get_mode');
      if strcmp(device_mode, 'gpu');
        diff = caffe_('blob_get_gpu_diff', self.hBlob_self);
      else
    ...
    function diff = get_gpu_diff(self)
      diff = caffe_('blob_get_gpu_diff', self.hBlob_self);
    end
