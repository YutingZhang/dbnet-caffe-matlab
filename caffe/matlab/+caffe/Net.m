classdef Net < handle
  ...
  methods
    function self = Net(varargin)
      ...
      % blob loss weights
      self.blob_loss_weights = self.attributes.blob_loss_weights;
    end
    ...
    function success_list = copy_from(self, varargin)
      success_list = caffe.copy_net_from( self, varargin{:} );
    ...
    function success_list = copy_blobs_from(self, varargin)
        success_list = caffe.copy_blobs_from( self, varargin{:} );
    end
    ...
    function reshape_input_blobs(self, input_shapes)
      CHECK(iscell(input_shapes), 'input_shapes must be a cell array');
      CHECK(length(input_shapes) == length(self.inputs), ...
        'input data cell length must match input blob number');
      % reshape input blobs
      for n = 1:length(self.inputs)
        if isempty(input_shapes{n})
            continue;
        end
        self.blobs(self.inputs{n}).reshape(input_shapes{n});
      end
      self.reshape();
    end
    function save(self, weights_file)
      CHECK(ischar(weights_file), 'weights_file must be a string');
      [pdir,~,~] = fileparts( weights_file );
