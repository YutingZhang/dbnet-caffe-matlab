...
static mxArray* blob_to_mx_mat(const Blob<float>* blob,
    WhichMemory data_or_diff, bool use_gpu_mat = false) {
  ...
  mxArray* mx_mat;
  float* mat_mem_ptr;
  mxGPUArray* mx_mat_gpu;
  if ( use_gpu_mat ) {
	  mx_mat_gpu = mxGPUCreateGPUArray(
			  dims.size(), dims.data(), mxSINGLE_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE) ;
	  mat_mem_ptr = reinterpret_cast<float*>(mxGPUGetData(mx_mat_gpu));
  } else {
	  mx_mat = mxCreateNumericArray(dims.size(), dims.data(), mxSINGLE_CLASS, mxREAL);
	  mat_mem_ptr = reinterpret_cast<float*>(mxGetData(mx_mat));
  }
  ...
  if ( use_gpu_mat ) {
	  mx_mat = mxGPUCreateMxArrayOnGPU( mx_mat_gpu );
      mxGPUDestroyGPUArray( mx_mat_gpu );
  }
  return mx_mat;
}
...
// Usage: caffe_('get_mode')
static void get_mode(MEX_ARGS) {
	 mxCHECK(nrhs == 0, "no argument needed" );
	 mxCHECK(nlhs <= 1, "exact one output" );
	 switch (Caffe::mode()) {
	 case Caffe::CPU:
...
static void solver_set_iter(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) &&
		  mxIsNumeric(prhs[1]) && mxIsScalar(prhs[1]),
      "Usage: caffe_('solver_set_iter', hSolver, iter)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  double iter = mxGetScalar(prhs[1]);
  solver->mutable_iter() = int(iter);
}
...
// Usage: caffe_('solver_get_snapshot_prefix', hSolver)
static void solver_get_snapshot_prefix(MEX_ARGS) {
	mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
		"Usage: caffe_('solver_get_max_iter', hSolver)");
	Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
	plhs[0] = mxCreateString(solver->param().snapshot_prefix().c_str());
}
...
// Usage: caffe_('solver_snapshot', hSolver, snapshot_fn_noext)
static void solver_snapshot(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) && mxIsChar(prhs[1]),
      "Usage: caffe_('solver_snapshot', hSolver, snapshot_fn_noext)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  string snapshot_fn_noext( mxArrayToString(prhs[1]) );
  solver->SetSnapshotName( snapshot_fn_noext.c_str() );
  solver->Snapshot();
  solver->SetSnapshotName( "" );
}

// Usage: caffe_('solver_type', hSolver)
static void solver_type(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('solver_type', hSolver)");
      Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
      const char* solver_type = solver->Type();
      mxArray* solver_type_ma = mxCreateString(solver_type);
      plhs[0] = solver_type_ma;
}
...
static void net_get_attr(MEX_ARGS) {
  ...
  mxSetField(mx_net_attr, 0, "blob_loss_weights",
      float_vec_to_mx_vec(net->blob_loss_weights()));
  plhs[0] = mx_net_attr;
}
...
// Usage: caffe_('blob_get_gpu_data', hBlob)
static void blob_get_gpu_data(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('blob_get_data', hBlob)");
  Blob<float>* blob = handle_to_ptr<Blob<float> >(prhs[0]);
  plhs[0] = blob_to_mx_mat(blob, DATA, true);
}
...
// Usage: caffe_('blob_get_gpu_diff', hBlob)
static void blob_get_gpu_diff(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('blob_get_diff', hBlob)");
  Blob<float>* blob = handle_to_ptr<Blob<float> >(prhs[0]);
  plhs[0] = blob_to_mx_mat(blob, DIFF, true);
}
...
static handler_registry handlers[] = {
  ...
  { "blob_get_gpu_data",      blob_get_gpu_data   },
  { "blob_get_gpu_diff",      blob_get_gpu_diff   },
  ...
  { "solver_get_snapshot_prefix",    solver_get_snapshot_prefix     },
  { "get_mode",				get_mode },
